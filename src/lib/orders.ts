// Shared helpers for working with orders — ID generation, consistent API serialization, and
// linking guest orders to a user account once they log in or register with a matching email.
// Centralising this avoids each API route (create, detail, list, track, admin) re-implementing
// its own slightly-different shape of the order JSON sent to the frontend.
 ...
 * verified email address (called right after login/register/Google sign-in). This is what makes
 * "Track Order" on the order-success page work for a guest who then creates an account, and keeps
 * order data correctly tied to the right user as the DB requirements ask for.
 */
export async function linkGuestOrdersToUser(userId: string, email: string) {
  const normalizedEmail = email.trim().toLowerCase();
  if (!normalizedEmail) return;

  // SQLite's `=` comparison is case-sensitive, so we scan guest orders (there are only ever a
  // handful per email) and match case-insensitively in JS rather than in the query.
