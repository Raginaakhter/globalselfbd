...
/**
 * Order confirmation email — sent right after an order (guest or logged-in) is successfully
 * placed. Includes the Order ID, full item/price breakdown, delivery info, and a tracking link
 * so the customer can look the order up again later with their email + Order ID.
 */
...
/**
 * Order status update email — sent whenever an admin changes an order's status (e.g. to
 * "Out for Delivery" or "Delivered"), so the customer doesn't have to keep checking the tracking
 * page manually.
 */
